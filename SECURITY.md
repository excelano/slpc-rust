...
**Rewriting a container keeps where it came from**, from 0.3.7. `repack` replaces a container by writing a new file and renaming it over the old one, and a new file carries no mark — so until 0.3.7, editing the metadata of a container that had been downloaded stripped the record of where it came from, and every payload unpacked from it afterwards was unmarked too. Replacing a container in place now carries the mark onto the replacement, and `repack -o` carries it onto the new file.
