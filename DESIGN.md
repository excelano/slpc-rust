...
Everything in §4.1 writes into a stream the caller supplies, which is the right shape for a library: a sink can be a file, a socket, a buffer, or a pipe, and a function that took a path instead would have no answer for the other three. What it leaves open is that the reading side has taken a path since 0.1.0, through `Container::open`, and the writing side has never had the equivalent. Every caller putting a container on disk has had to supply that half itself.

`Destination`, behind the `fs` feature, is that half. It writes through a temporary file beside the destination and renames it into place, so a write that fails partway leaves nothing behind rather than a truncated container that looks like one, and replacing a file cannot destroy the old one before failing to produce the new. It lends out a `File`, which is the `Write + Seek` §4.4 asks for and lets a caller read back its own output before anything is replaced by it.

**It is a feature rather than part of the default surface.** A caller who only reads containers, or who writes into a socket, should not acquire a temporary-file dependency to do it, and §3's fifteen-crate tree is a claim worth keeping true for them. Both binaries in this workspace turn it on, and both were already carrying those crates.

**Why the library rather than each binary.** The code has no format knowledge in it, which was the argument for keeping it out — until the same test is applied to `Container::open`, which has none either and belongs here on a convention argument rather than a format one. The rule the library actually held was that it reads from paths and never writes to them, and that is an asymmetry rather than a principle. The stronger reason is what the code contains: files came out `0600` from 0.1.0 to 0.3.0 because a rename carries a private mode, and the fix is not something a competent implementer derives on the spot. Held once, it is a defect fixed; copied into each program that writes a container, it is a defect rediscovered. There are two such programs in this workspace and a third in `excelano/slipcase-desktop`.

**Two mode policies, deliberately.** `Destination::new` creates a file at a path the caller named and gives it what the umask decided, whether or not `force` replaced something that was in the way — a file that happened to be there does not get to decide who can read what replaces it. `Destination::in_place` replaces a particular file, resolves symbolic links first so the file is replaced rather than the link, and carries that file's own permissions across. What a rename cannot carry is ownership, which is the standing cost of replacing a file rather than writing into it, and it is shared with `sed -i` and with every editor's default.
...
- **Writing in place resolves the path first**, so a container reached through a symbolic link is replaced rather than the link being replaced by a file, and the container's permissions are carried onto the temporary file before the rename. Both are §4.7's, reached by asking the library for an in-place destination rather than a new one; the tool chooses which, and the library knows what each means.
- **A file this tool creates comes out with the permissions any other new file would have**, and that is the library's doing rather than this tool's. §4.7 holds the temporary file, the mode, and the rename, because the same knowledge is owed to every program that writes a container and it was wrong here for three releases. What stays in the tool is what is shaped like a command line: `-` for standard output, refusing to write a ZIP at a terminal, and the wording of the messages — the library reports that a destination exists and says nothing about `--force`, having no flags of its own.
