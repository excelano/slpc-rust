...
**One branch is not reachable by any test on an unsandboxed machine, and a `cfg(test)` switch denies the quarantine write alone to reach it.** The sandbox denies one attribute and permits every other write to the same file, so the existing `unwritable` helper cannot stand in: `0o444` denies our own attribute exactly as it denies quarantine and can only ever produce `AlreadyMarked`. Nor can a malformed value, because the kernel validates it not at all — an empty one, `garbage` and non-hex flags are all accepted. A branch that decides whether provenance survives a save is not one to leave uncovered because the platform makes it awkward.
...
every defect the security review found came from somebody *reasoning*
...
Sixteen million cases across eight seeds, then sixteen million
...
The corpus is built upstream rather than with this implementation: a corpus produced by the tool it validates proves nothing. How it is built is the specification repository's business to describe. Generated fixtures here cover only what the corpus does not.
