...
**Amended 2026-08-28: `AlreadyMarked` was losing the answer to *where did it come from*, and the value can be kept beside the platform's mark.** The paragraph above is right that the gate is in place and that the source's value is detail rather than a control. What it did not follow through is what the detail is for. Under the App Sandbox the platform overwrites the source's mark with one naming the writing process, so a caller that reports provenance — `slipcase-desktop`'s card is the one that does — stops being able to tell a container that arrived from elsewhere from one made on the machine. Measured by hand on 2026-08-28: saving an edit to a downloaded container succeeded, the file stayed gated with its flags unchanged at `0083`, the agent became `slipcase-desktop`, and the card's *arrived from elsewhere* line was there before the save and gone after it. Nothing was less safe and the application had quietly stopped telling the truth.

That was taken as unavoidable for most of a day, and it is not. What is unavoidable is *attribution through the platform*: measured inside a bundle signed with the sandbox entitlement, a raw `setxattr` of `com.apple.quarantine` fails with `EPERM`, and Apple's supported route — `NSURL`'s `quarantinePropertiesKey` — succeeds and then substitutes its own agent, with the supplied origin never reaching `com.apple.LaunchServices.QuarantineEventsV2`. A sandboxed process cannot attribute a file to anyone but itself. Safari manages it only by way of a broker running outside its own sandbox, which a Mac App Store application cannot have.
...
**Windows can reach the same branch and deliberately does nothing yet.** A second alternate data stream would hold a note perfectly well. What is missing is a measurement — what the shell does with an unknown stream, whether it survives the copies and archivers that strip `Zone.Identifier`, whether a packaged install sees it — and writing it on the strength of the macOS result would be the inference this document keeps refusing to make. Linux cannot reach the branch at all, since its `carry` never fails.

**One branch here is not reachable by any test on an unsandboxed machine, and it has a seam rather than a gap.** The sandbox denies one attribute and permits every other write to the same file. Measured 2026-08-28: `0o444` denies our own attribute exactly as it denies quarantine, so the existing `unwritable` helper can only ever produce `AlreadyMarked`; and the kernel validates the value not at all — an empty one, `garbage`, and non-hex flags are all accepted — so a malformed mark is no seam either. A `cfg(test)` switch denies the quarantine write alone. A branch that decides whether provenance survives a save is not one to leave uncovered because the platform makes it awkward.
...
**1 MiB, and why the number has to be chosen against the parsed cost.** `toml_edit` keeps a key's decor, span and representation alongside its value so a rewrite can put back what it did not touch, so the parsed document is not a small multiple of its source. Measured on 2026-08-27 against the densest conformant shape — shortest legal keys, shortest legal values — 256 KiB of metadata parses to 22 MB resident and 1 MiB to 85 MB. Around 85 times, and the multiplier is a function of how many keys fit rather than of the size, so the dense shape is the one to choose the number against.
