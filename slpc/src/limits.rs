...
//
// SPEC 6 requires a bound on the flyleaf member and names no number, because
...
/// a reader must decompress the flyleaf member and parse it as TOML before it
...
/// [`flyleaf_of_with`](crate::flyleaf_of_with). The unsuffixed forms of all
...
/// limits.flyleaf_bytes = 1 << 20;
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub struct Limits {
    /// The most the flyleaf member may decompress to, in bytes.
    ///
    /// A container whose flyleaf member exceeds this is reported
    ...
    /// conformance.
    pub flyleaf_bytes: u64,
}

impl Limits {
    /// The default bound on the flyleaf member: 1 MiB.
    ///
    ...
    /// shortest legal values — a 256 KiB flyleaf parses to 22 MB resident and
    ...
    /// conformance corpus holds a 64 KiB flyleaf. A megabyte is generous
    ...
    /// lower still.
    pub const DEFAULT_FLYLEAF_BYTES: u64 = 1 << 20;
}
...
impl Limits {
    ...
    pub const DEFAULT: Self = Self {
        flyleaf_bytes: Self::DEFAULT_FLYLEAF_BYTES,
    };
}
