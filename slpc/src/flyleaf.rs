// The flyleaf member: reading the two keys SPEC 2.2 requires, and checking a
...
use crate::{CONTENT_FILE_KEY, VERSION_KEY};
...
/// Parse a flyleaf member and read both required keys.
///
...
pub(crate) fn parse(bytes: &[u8]) -> Result<(DocumentMut, Keys)> {
    ...
    let keys = Keys {
        ...
        content_file: required_string(&doc, CONTENT_FILE_KEY)?.to_owned(),
    };
    Ok((doc, keys))
}

/// Parse a flyleaf member as TOML, and ask nothing else of it.
///
...
/// SPEC 2.1 somewhere else entirely and still have a flyleaf document worth
/// reading.
pub(crate) fn document(bytes: &[u8]) -> Result<DocumentMut> {
    let text = std::str::from_utf8(bytes).map_err(|_| Malformed::FlyleafNotUtf8)?;
    text.parse()
        .map_err(|e: toml_edit::TomlError| Malformed::FlyleafNotToml(e.to_string()).into())
}

/// Follow a key through the document.
///
/// The key may be dotted, as `content.file` is, in which case each part is a
...
/// `content file = { file = "..." }`. That is valid TOML and a conformant container,
...
///
/// The error names `content` because that is the only intermediate there is:
...
pub(crate) fn set(doc: &mut DocumentMut, key: &str, to: &str) -> Result<()> {
    ...
    for part in tables {
        match at.get(part) {
            ...
            Some(_) => return Err(Malformed::ContentNotATable.into()),
        }
        at = &mut at[*part];
    }
